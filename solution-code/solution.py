...
def compare_models(prompt: str) -> dict:
    ...
    """
    resp_4o, lat_4o, use_4o = call_openai(prompt=prompt, model="gpt-4o")
    resp_4o_mini, lat_4o_mini, use_4o_mini = call_openai(prompt=prompt, model="gpt-4o-mini")
    resp_gemini, lat_gemini, use_gemini = call_gemini(prompt=prompt, model="gemini-2.5-flash")
    
    cost_4o = (use_4o["input_tokens"] * PRICING_1M_TOKENS["gpt-4o"]["input"] + use_4o["output_tokens"] * PRICING_1M_TOKENS["gpt-4o"]["output"]) / 1_000_000
    cost_4o_mini = (use_4o_mini["input_tokens"] * PRICING_1M_TOKENS["gpt-4o-mini"]["input"] + use_4o_mini["output_tokens"] * PRICING_1M_TOKENS["gpt-4o-mini"]["output"]) / 1_000_000
    cost_gemini = (use_gemini["input_tokens"] * PRICING_1M_TOKENS["gemini-2.5-flash"]["input"] + use_gemini["output_tokens"] * PRICING_1M_TOKENS["gemini-2.5-flash"]["output"]) / 1_000_000
    
    return {
        "gpt4o": {"response": resp_4o, "latency": lat_4o, "cost": cost_4o, "input_tokens": use_4o["input_tokens"], "output_tokens": use_4o["output_tokens"]},
        "gpt4o_mini": {"response": resp_4o_mini, "latency": lat_4o_mini, "cost": cost_4o_mini, "input_tokens": use_4o_mini["input_tokens"], "output_tokens": use_4o_mini["output_tokens"]},
        "gemini_flash": {"response": resp_gemini, "latency": lat_gemini, "cost": cost_gemini, "input_tokens": use_gemini["input_tokens"], "output_tokens": use_gemini["output_tokens"]}
    }
...
def streaming_chatbot() -> None:
    ...
    while True:
        try:
            ...
            messages.append(types.Content(role="model", parts=[types.Part.from_text(text=full_response)]))
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for row in results:
        prompt_trunc = row["prompt"][:50].replace('\n', ' ')
        if len(row["prompt"]) > 50: 
            prompt_trunc += "..."
        
        for model_id, model_name in [("gpt4o", "GPT-4o"), ("gpt4o_mini", "GPT-4o-Mini"), ("gemini_flash", "Gemini-Flash")]:
            if model_id in row:
                m_data = row[model_id]
                resp_trunc = m_data["response"][:50].replace('\n', ' ')
                if len(m_data["response"]) > 50: 
                    resp_trunc += "..."
                
                lines.append(
                    f"| {prompt_trunc} | {model_name} | {resp_trunc} | "
                    f"{m_data['latency']:.2f}s | {m_data['input_tokens']}/{m_data['output_tokens']} | "
                    f"${m_data['cost']:.6f} |"
                )
    return "\n".join(lines)
